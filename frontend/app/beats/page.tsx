...
export default function BeatsPage() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        const data = await apiRequest<Beat[]>("/beats/");
        setBeats(data);
      } catch (err) {
    ...
    <div className="space-y-6">
                ...
                >
                  <div className="flex items-start gap-3">
                    <button
                      ...
                      className={`mt-1 h-10 w-10 rounded-full border text-sm ${isCurrent && isPlaying ? "border-[#8b28ff] bg-[#8b28ff] text-white" : "border-white/20 bg-white/5 text-white/85"}`}
                    >
      ...
      {licenseModalBeat ? (
        <div className="fixed inset-0 z-[130] flex items-start justify-center bg-black/70 px-4 pt-24 backdrop-blur-sm" onClick={() => setLicenseModalBeat(null)}>
          <section className="w-full max-w-[960px] rounded-2xl border border-white/15 bg-[#1d1f2a] p-5" onClick={(e) => e.stopPropagation()}>
            <div className="grid gap-5 lg:grid-cols-[320px_1fr]">
                ...
                <div className="overflow-hidden rounded-xl border border-white/10">
                  {licenseOptions.map((item) => (
                    <button
                      ...
                      className={`block w-full border-b border-white/10 px-4 py-3 text-left text-2xl ${selectedLicense === item.key ? "bg-[#4593f5] text-white" : "bg-transparent text-white/90"}`}
                    >
                      {item.label}
                    </button>
                  ))}
              ...
              <div>
                <button type="button" onClick={() => setLicenseModalBeat(null)} className="float-right text-2xl text-white/70">x</button>
                <div className="mb-4 flex items-center gap-3">
                  <div className="flex h-20 w-20 items-center justify-center rounded-md border border-white/10 bg-gradient-to-br from-[#2a3546] to-[#11151d] text-sm font-bold text-white/80">
                    {licenseModalBeat.title.slice(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <p className="text-4xl font-semibold">{licenseModalBeat.title}</p>
                    <p className="text-xl text-white/70">{licenseModalBeat.producer_username}</p>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-white/10 bg-black/20 p-4 text-lg">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <p className="text-white/70">Publishing Rights</p><p className="text-right">{selectedLicense === "exclusive" ? (licenseModalBeat.exclusive_publishing_rights || "0%") : (licenseModalBeat.non_exclusive_publishing_rights || "0%")}</p>
                ...
                <div className="mt-4 flex justify-end">
                  <button type="button" className="brand-btn px-8 py-3 text-3xl font-semibold">
