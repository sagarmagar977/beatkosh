...
class PaymentsApiTests(APITestCase):
    ...
    def test_cart_me_returns_comma_formatted_display_amounts(self):
        premium_beat = Beat.objects.create(
            producer=self.producer,
            title="Premium Cart Beat",
            genre="Trap",
            bpm=140,
            base_price="13800.00",
        )
        premium_beat.available_licenses.add(self.license)

        add_response = self.client.post(
            reverse("cart-item-create"),
            {
                "product_type": "beat",
                "product_id": premium_beat.id,
                "license_id": self.license.id,
            },
            format="json",
        )
        ...
        self.assertEqual(cart_response.data["beat_total_display"], "13,850.00")
        self.assertEqual(cart_response.data["subtotal_display"], "13,850.00")
        self.assertEqual(cart_response.data["total_display"], "13,850.00")
        self.assertIn("13,800.00", [item["price_display"] for item in cart_response.data["items"]])
    ...
    def test_esewa_complete_verifies_payment_and_grants_access(self, mock_status_check):
        payment_response = self.client.post(
            reverse("payment-initiate"),
